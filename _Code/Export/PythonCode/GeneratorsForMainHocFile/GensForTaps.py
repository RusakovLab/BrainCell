...
class GensForTaps:
    ...
    def createTemplatesForGapJuncs(self):
        ...
        if any(self._isPtrStubInExtGapJuncStrucOfAnySet(gapJuncSet) for gapJuncSet in hocObj.gjmAllGapJuncSets):
    ...
    def createTemplatesForSyns(self):
        ...
        is3PartsInSynStrucOfAnySet = any(synSet.is3Or1PartInSynStruc() for synSet in hocObj.smAllSynSets)
    ...
    def createGapJuncLocs(self, gapJuncSetIdx):
        ...
        gapJuncSet = hocObj.gjmAllGapJuncSets[gapJuncSetIdx]
    ...
    def createSynLocs(self, synSetIdx):
        ...
        synSet = hocObj.smAllSynSets[synSetIdx]
    ...
    # Keep in sync with hoc:EnumGapJuncCompIdxs.init, hoc:GapJuncSet.init and py:GensForHomogenVars.initHomogenGapJuncVars
    ...
    # Keep in sync with hoc:EnumSynCompIdxs.init, hoc:SynSet.init and py:GensForHomogenVars.initHomogenSynVars
    def createReducedSynComps(self, synSetIdx):
        
        lines = []
        
        synSet = hocObj.smAllSynSets[synSetIdx]
        enumSynPpRoles = hocObj.enumSynPpRoles
    ...
    def initHomogenGapJuncVars(self, gapJuncSetIdx):
        ...
        mechIdxs[1] = int(hocObj.utils4FakeMech4GapJuncExtValue.mechIdx)    # "External Value"
        ...
        return self._gensForHomogenVars.initHomogenTapVars( \
            ...
            hocObj.exportOptions.isExportGapJuncAssignedAndState, \
            hocObj.exportOptions.isExportGapJuncInhoms)
            
    # Keep in sync with hoc:EnumSynCompIdxs.init, hoc:SynSet.init and py:GensForTaps.createReducedSynComps
    def initHomogenSynVars(self, synSetIdx):
        ...
        mechIdxs[1] = int(hocObj.utils4FakeMech4SynNetCon.mechIdx)                  # "NetCon"
        ...
        return self._gensForHomogenVars.initHomogenTapVars( \
            ...
            hocObj.exportOptions.isExportSynAssignedAndState, \
            hocObj.exportOptions.isExportSynInhoms)
    ...
    def createMainPartForGapJuncs(self, gapJuncSetIdx):
        ...
        mechName = h.ref('')
        mechIdx = int(gapJuncSet.getMechIdxAndOptionalName(mechName))
        mechName = mechName[0]
        
        fixedMechIdx = int(hocObj.utils4FakeMech4GapJuncExtValue.mechIdx)
        
        pyObj = hocObj.pyObj    # !!!!! or import ?
        ...
        lines.append('{{ _gapJuncSet.createGapJuncStruc("{}", {}, "{}", "{}") }}'.format( \
            ...
            ppPtrName, \
            dmVarNameWithIndex))
        lines.append('{{ _gapJuncSet.initAllHomogenVars({}, {}) }}'.format(mechIdx, fixedMechIdx))
        lines.append('')
    ...
    def createMainPartForSyns(self, synSetIdx):
        ...
        trgMechName = h.ref('')
        sngMechName = h.ref('')
        srcMechIdx = int(synSet.getMechIdxAndOptionalName(enumSynPpRoles.srcPp, srcMechName))
        trgMechIdx = int(synSet.getMechIdxAndOptionalName(enumSynPpRoles.trgPp, trgMechName))
        sngMechIdx = int(synSet.getMechIdxAndOptionalName(enumSynPpRoles.sngPp, sngMechName))
        ...
        lines.append('')
        lines.append('{{ _synSet.createSynStruc({}, {}, "{}", "{}", "{}") }}'.format( \
            is3Or1PartInSynStruc, \
            srcMechIdx, \
            srcMechName, \
            trgMechName, \
            sngMechName))
        lines.append('{{ _synSet.initAllHomogenVars({}, {}, {}, {}) }}'.format( \
            is3Or1PartInSynStruc, \
            srcMechIdx, \
            trgMechIdx, \
            sngMechIdx))
    ...
    def _appendNewReducedPPComp(self, lines, isGapJuncOrSyn, compName, enumPpRoleStr, mechIdxOrMinus1):
        if mechIdxOrMinus1 != -1:
            ...
            comment = ''
        lines.append('{{ _allComps.append(new ReducedPPComp({}, "{}", {}, {})) }}{}'.format(isGapJuncOrSyn, compName, enumPpRoleStr, mechIdxOrMinus1, comment))
    ...
    def _appendNewReducedFakeComp(self, lines, isGapJuncOrSyn):
        ...
        if isGapJuncOrSyn:
            if exportOptions.isExportAnyInhomExtValueGapJuncModels():
                ...
                ctorNameWithArgs = 'ReducedFakeComp(enumGapJuncPpRoles.extValue, {})'.format(int(hocObj.utils4FakeMech4GapJuncExtValue.mechIdx))
        else:
            if exportOptions.isExportAnyInhomNetConSynModels():
                ctorNameWithArgs = 'ReducedSynNCComp()'
            else:
                ctorNameWithArgs = 'ReducedFakeComp(enumSynPpRoles.netCon, {})'.format(int(hocObj.utils4FakeMech4SynNetCon.mechIdx))
