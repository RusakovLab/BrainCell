...
class MetaGensForTaps:
    ...
    def createAllGapJuncSets(cls):
        if hocObj.exportOptions.isExportAnyGapJuncSets():
    ...
    def createAllSynSets(cls):
        if hocObj.exportOptions.isExportAnySynSets():
    ...
    def _metaUnwrapAllTapSets(cls, isGapJuncOrSyn):
        ...
        if isGapJuncOrSyn:
            gapJuncOrSynStr = 'GJ'
            xmAllTapSets = hocObj.gjmAllGapJuncSets
        else:
            gapJuncOrSynStr = 'syn'
            xmAllTapSets = hocObj.smAllSynSets
        ...
        for tapSetIdx in range(len(xmAllTapSets)):
            for line in lines:
                if isGapJuncOrSyn:
                    ...
                    tapSetIdx1Based = int(hocObj.smAllSynSets[tapSetIdx].idxForSynSet)
